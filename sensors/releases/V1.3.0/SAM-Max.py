...
class SensorGUI:
    def __init__(self, root):
        self.root = root
        # GUI Setup
        self.root.title("Stork Aquatics Monitor Max V1.3")
    ...
    def open_settings_popup(self, sensor_id):
        ...
        if use_liters_var.get() and width > 0 and depth > 0:
            on_entry.insert(0, f"{(on_val * width * depth / 1000):.2f}")
            off_entry.insert(0, f"{(off_val * width * depth / 1000):.2f}")
        elif use_gallons_var.get() and width > 0 and depth > 0:
            on_entry.insert(0, f"{(on_val * width * depth / 1000 * 0.264172):.2f}")
            off_entry.insert(0, f"{(off_val * width * depth / 1000 * 0.264172):.2f}")
        else:
        ...
        def save_thresholds():
            try:
                ...
                if use_liters_var.get():
                    mmwg_on = (on_val * 1000) / (width * depth)
                    mmwg_off = (off_val * 1000) / (width * depth)
                elif use_gallons_var.get():
                    liters_on = on_val / 0.264172
                    liters_off = off_val / 0.264172
                    mmwg_on = (liters_on * 1000) / (width * depth)
                    mmwg_off = (liters_off * 1000) / (width * depth)
                else:
                    mmwg_on = on_val
                    mmwg_off = off_val
    ...
    def open_graphics_popup(self):
        ...
        popup.attributes("-fullscreen", True)
        popup.bind("<Double-Button-1>", lambda event: popup.attributes("-fullscreen", not popup.attributes("-fullscreen")))
        dark_mode = self.visual_settings.get("darm_mode", False)
