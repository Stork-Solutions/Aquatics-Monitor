# Stork Aquatics Monitor Max V1.3.7
# Wi-Fi TCP - [Enabled]
# 4Ch Relay - [Enabled]
...
class SensorGUI:
    def __init__(self, root):
        self.root = root
        # GUI Setup
        self.root.title("Stork Aquatics Monitor Max V1.3.7")
        ...
        self.spare_frame_a = self.create_spare_frame_a("Spare A", 2, 0)
    ...
    def apply_frame_visibility(self):
        ...
        try:
            mapping.update({
                ...
                "Spare A": self.spare_frame_a["frame"],
                "": self.image_frame_b["frame"],
                "www.stork.solution": self.image_frame_c["frame"],
            })
        except Exception:
    ...
    def create_spare_frame_a(self, title, row, column, colspan=1):
        frame = tk.LabelFrame(self.root, text=title or "", font=("Arial", 16, "bold"), padx=10, pady=10)
        frame.grid(row=row, column=column, padx=10, pady=10, sticky="nsew", columnspan=colspan)

        tk.Label(frame, text="(Spare frame – ready to use)", font=("Arial", 12, "italic")).pack(pady=8)
