# Sensor ID "A"
# ME782-R-0P5-0-9-1-0-0 0.5Bar Gauge / -40 / +125°C
# Stork Aquatics Level & Temperature Sensor MK2 V1.0.6
import os
import machine
import network
import socket
...
# Sensor Reads
ME782_OUT_REG = 0x78      # Output register for P/T (digital 15-bit each)
ME782_ADDR = 0x78         # discovered at runtime via i2c.scan()

# Sensor ranges (head-specific)
P_MIN_BAR = 0.0
P_MAX_BAR = 0.5           # 0–0.5 bar gauge head
T_MIN_C = -40.0
T_MAX_C = 125.0

# Transfer-function constants (datasheet)
AP, BP = 0.05, 0.95       # pressure uses 5–95% of 15-bit span
AT, BT = 0.10, 0.90       # temperature uses 10–90% of 15-bit span

def _me782_discover():
    """Scan the I²C bus and set ME782_ADDR if a compatible device responds."""
    global ME782_ADDR
    if ME782_ADDR is not None:
        return True
    try:
            ...
            try:
                ...
                if len(raw) == 4:
                    ...
                    if (p_raw | t_raw) != 0 and 0 <= p_raw <= 32767 and 0 <= t_raw <= 32767:
                        ME782_ADDR = addr
                        try:
                            print("ME782 detected at I2C 0x%02X" % addr)
                        except Exception:
                            pass
                        return True
            except Exception:
...
def _me782_read_raw():
    """Return (Pdec, Tdec) as 15-bit ints [0..32767]. Raises on failure."""
    raw = i2c.readfrom(ME782_ADDR, 4)
    p_raw = ((raw[0] << 8) | raw[1]) & 0x7FFF
    t_raw = ((raw[2] << 8) | raw[3]) & 0x7FFF
    return p_raw, t_raw
...
def read_pressure():
    """Return level in mmWG (string, 1 dp) or 'ERR'."""
    try:
        if not _me782_discover():
            return "ERR"
        pdec, _ = _me782_read_raw()
        p_bar = _me782_convert_pressure_bar(pdec)
        mmwg = p_bar * 10197.16   # 1 bar = 10,197.16 mmWG
        return str(round(mmwg, 1))
    except Exception as e:
        try:
            print("ME782 pressure read error:", e)
        except Exception:
            pass
        return "ERR"

def read_temperature():
    """Return temperature in °C (string, 1 dp) or 'ERR'."""
    try:
        if not _me782_discover():
            return "ERR"
        ...
        try:
            print("ME782 temperature read error:", e)
        except Exception:
            pass
        return "ERR"
...
# === New helper: persist IP lock (learn DHCP then pin static) ===
...
def connect_wifi():
    ...
    if wlan.isconnected():
        ...
        update_assigned_ip(ip)              # keep original diagnostic write
        persist_ip_lock_from_wlan(wlan)     # NEW: learn & lock full tuple
        blue_led.on()
    else:
