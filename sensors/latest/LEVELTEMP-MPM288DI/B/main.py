...
def ensure_ap_up():
    ...
    if not ap.active():
        ap.active(True)
    ap.config(essid='Sensor-B', password='sensor1234')
    print("System Ok")
...
<title>Sensor B — Wi-Fi Setup</title>
...
def run_ap_status_server():
    ...
    try:
        s.bind(('0.0.0.0', 80)) 
        s.listen(4)
        print("AP status page on http://%s:80 (connect to Sensor-B Wi-Fi)" % ap_ip)
    except Exception as e:
...
<title>Sensor B — Connected</title>
<style>
