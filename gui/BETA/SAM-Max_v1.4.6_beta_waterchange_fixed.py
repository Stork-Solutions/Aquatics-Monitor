...
class SensorGUI:
    ...
    def attach_water_change_button(self, frame, command):
        ...
        Uses plain text (WC) to avoid emoji font issues on some Raspberry Pi setups.
        """
        btn = tk.Button(
            frame,
            text="WC",
            font=("Arial", getattr(self, "_s", lambda x: x)(11), "bold"),
            command=command,
            bd=0,
            relief="flat",
            highlightthickness=0
        )
        # Place slightly left of the corner so it doesn't overlap other overlay buttons.
        btn.place(relx=1.0, rely=0.0, anchor="ne", x=-44, y=6)
        return btn
    # Auto GitHub Update
    ...
    def open_water_change_popup(self, default_sensor="A"):
        """Popup to start an automated drain (water change)."""
        try:
            ...
            tk.Button(btn_row, text="Cancel", font=("Arial", 12), command=_cancel).grid(row=0, column=0, sticky="ew", padx=(0, 8))
            tk.Button(btn_row, text="Submit", font=("Arial", 12, "bold"), command=_submit).grid(row=0, column=1, sticky="ew", padx=(8, 0))
