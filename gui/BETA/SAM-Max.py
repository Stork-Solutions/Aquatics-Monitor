...
class SensorGUI:
    def __init__(self, root):
        ...
        try:
            if os.path.exists(self._settings_path):
                with open(self._settings_path, "r") as _f:
                    ...
                    # Backward compatibility: older builds used "5" for the mid-size profile
                    if self.screen_profile == "5":
                        self.screen_profile = "7"
        except Exception:
            self.screen_profile = None
        ...
        if self.screen_profile not in ("4.3", "7", "10.1"):
            self.screen_profile = _detect_screen_profile(screen_w, screen_h)

        self.SCREEN_PROFILES = {
            "4.3":  {"columns": 1, "outer_pad": 4,  "pady": 6,  "scale": 0.85, "force_fullscreen": True, "base_res": (480, 272),  "auto_scale": True},
            "7":    {"columns": 2, "outer_pad": 6,  "pady": 8,  "scale": 1.00, "force_fullscreen": True, "base_res": (800, 480),  "auto_scale": True},
            "10.1": {"columns": 3, "outer_pad": 10, "pady": 10, "scale": 1.25, "force_fullscreen": True, "base_res": (1280, 800), "auto_scale": True},
        }
        self.profile = self.SCREEN_PROFILES[self.screen_profile]

        # Clamp to avoid extremes
        # Base UI scale for the selected screen profile
        _base_scale = float(self.profile.get("scale", 1.0))
        # Optional auto-scaling against a reference resolution (helps distinguish 800x480 vs 1024x600 etc.)
        if self.profile.get("auto_scale"):
            try:
                _bw, _bh = self.profile.get("base_res", (screen_w, screen_h))
                _factor = min(float(screen_w) / float(_bw), float(screen_h) / float(_bh))
                _base_scale *= _factor
            except Exception:
                pass

        # Clamp to avoid extremes
        self.scale = max(0.7, min(1.6, float(_base_scale)))
