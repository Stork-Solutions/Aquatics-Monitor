...
class SensorGUI:
    def __init__(self, root):
        ...
        self.root.after(5000, self.schedule_water_change_tick)  # Start scheduler 5s after launch
        ...
        self.water_change_schedule = {"enabled": False}
        self.root.after(60000, self.schedule_water_change_tick)
...
# Auto GitHub Update
    ...
    def stop_flashing(self, pump_name):
        ...
        if pump_name == "RO Pump A":
            ...
            # Only restore "Connected" if no RO alarm currently active
            try:
            ...
            # Only restore "Connected" if no pH/temp alarm currently active
            try:
            ...
            # Only restore "Connected" if no TDS/temp alarm currently active
            try:
    ...
    # -----------------------------
    # Auto Water Change (Drain only)
    # -----------------------------
    
    def schedule_water_change_tick(self):
        """Background loop: checks every 30s for a pending scheduled water change."""
        print("[SCHEDULER] Tick fired.")
        try:
            sched = getattr(self, "water_change_schedule", {})
            print(f"[SCHEDULER] Schedule state: {sched}")

            if not sched.get("enabled", False):
                print("[SCHEDULER] No active schedule.")
            else:
                when_str = sched.get("when", "")
                if not when_str:
                    print("[SCHEDULER] Schedule enabled but no 'when' value found.")
                else:
                    try:
                        scheduled_time = datetime.datetime.strptime(when_str, "%Y-%m-%d %H:%M:%S")
                        now = datetime.datetime.now()
                        print(f"[SCHEDULER] Now: {now}  |  Scheduled: {scheduled_time}  |  Due: {now >= scheduled_time}")

                        if now >= scheduled_time:
                            sid = sched.get("tank", "A")
                            remove_mmwg = sched.get("remove_mmwg", 0)
                            print(f"[SCHEDULER] Time reached! Tank={sid}, Remove={remove_mmwg} mmWG")

                            if self.water_change_active.get(sid, False):
                                print(f"[SCHEDULER] Tank {sid} already draining. Skipping.")
                            else:
                                current = self.cached_readings.get(sid, {}).get("water_mmwg", None)
                                print(f"[SCHEDULER] Current reading for Tank {sid}: {current}")
                                if current is None:
                                    print(f"[SCHEDULER] No valid reading for Tank {sid}. Cannot trigger yet.")
                                    self.safe_gui_update(lambda sid=sid: self.show_error(
                                        "Scheduled Water Change Failed",
                                        f"Tank {sid}: No valid sensor reading available.\n\nThe scheduled water change could not start.\nPlease check the sensor connection and reschedule."
                                    ))
                                    self.water_change_schedule = {"enabled": False}
                                    self.save_threshold_settings()
                                else:
                                    print(f"[SCHEDULER] TRIGGERING water change for Tank {sid}!")
                                    self.start_water_change(sid, remove_mmwg)
                                    self.water_change_schedule = {"enabled": False}
                                    self.save_threshold_settings()
                                    print("[SCHEDULER] Schedule cleared after trigger.")
                        else:
                            remaining = (scheduled_time - now).total_seconds()
                            print(f"[SCHEDULER] Not yet due. {remaining:.0f}s remaining.")
                    except Exception as e:
                        print(f"[SCHEDULER] Inner error: {e}")
        except Exception as e:
            print(f"[SCHEDULER] Outer error: {e}")

        # Re-schedule in 30 seconds
        self.root.after(30000, self.schedule_water_change_tick)


    def open_water_change_popup(self, default_sensor="A"):
        """Popup to start/monitor an automated drain (water change) with scheduling."""
        win = tk.Toplevel(self.root)
        win.title("Auto Water Change")
        try:
        ...
        container = tk.Frame(win, bg=getattr(self, "bg_color", None))
        ...
        # Tank Selection
        tk.Label(
            container, text="Select Tank:", font=("Arial", 12, "bold"),
            bg=getattr(self, "bg_color", None), fg=getattr(self, "fg_color", None)
        ).grid(row=0, column=0, sticky="w", pady=(0, 6))

        tank_var = tk.StringVar(value=(default_sensor or "A"))
        tank_opts = ["A", "B"]
        tank_menu = tk.OptionMenu(container, tank_var, *tank_opts)
        tank_menu.config(font=("Arial", 12))
        tank_menu.grid(row=0, column=1, sticky="ew", pady=(0, 6), padx=(10, 0))

        # Amount
        tk.Label(
            container, text="Amount to remove (mmWG):", font=("Arial", 12, "bold"),
            bg=getattr(self, "bg_color", None), fg=getattr(self, "fg_color", None)
        ).grid(row=1, column=0, sticky="w", pady=(0, 10))

        amount_var = tk.StringVar(value="")
        amount_entry = tk.Entry(container, textvariable=amount_var, font=("Arial", 12))
        amount_entry.grid(row=1, column=1, sticky="ew", pady=(0, 10), padx=(10, 0))

        # Mode Selection (Do it now / Schedule)
        mode_frame = tk.Frame(container, bg=getattr(self, "bg_color", None))
        mode_frame.grid(row=2, column=0, columnspan=2, sticky="w", pady=(0, 10))

        do_now_var = tk.BooleanVar(value=True)
        sched_var = tk.BooleanVar(value=False)

        def _toggle_now():
            if do_now_var.get():
                sched_var.set(False)
                sched_row.grid_remove()
            else:
                sched_var.set(True)
                sched_row.grid()
        ...
        tk.Checkbutton(
            mode_frame, text="Do it now", variable=do_now_var, command=_toggle_now,
            font=("Arial", 11, "bold"), bg=getattr(self, "bg_color", None), 
            fg=getattr(self, "fg_color", None), selectcolor=getattr(self, "bg_color", None)
        ).pack(side="left", padx=(0, 20))

        tk.Checkbutton(
            mode_frame, text="Schedule", variable=sched_var, command=_toggle_sched,
            font=("Arial", 11, "bold"), bg=getattr(self, "bg_color", None), 
            fg=getattr(self, "fg_color", None), selectcolor=getattr(self, "bg_color", None)
        ).pack(side="left")

        # Schedule Options Row (Hidden by default)
        sched_row = tk.Frame(container, bg=getattr(self, "bg_color", None))
        sched_row.grid(row=3, column=0, columnspan=2, sticky="ew", pady=(0, 10))
        sched_row.grid_remove()

        tk.Label(sched_row, text="When:", font=("Arial", 11, "bold"), bg=getattr(self, "bg_color", None), fg=getattr(self, "fg_color", None)).pack(side="left", padx=(0, 5))

        day_var = tk.StringVar(value="Today")
        day_opts = ["Today", "Tomorrow", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
        day_menu = tk.OptionMenu(sched_row, day_var, *day_opts)
        day_menu.config(font=("Arial", 10))
        day_menu.pack(side="left", padx=(0, 5))

        hour_var = tk.StringVar(value=datetime.datetime.now().strftime("%H"))
        hour_opts = [f"{i:02d}" for i in range(24)]
        hour_menu = tk.OptionMenu(sched_row, hour_var, *hour_opts)
        hour_menu.config(font=("Arial", 10))
        hour_menu.pack(side="left", padx=(0, 5))

        min_var = tk.StringVar(value="00")
        min_opts = ["00", "15", "30", "45"]
        min_menu = tk.OptionMenu(sched_row, min_var, *min_opts)
        min_menu.config(font=("Arial", 10))
        min_menu.pack(side="left")

        # Pre-populate status label with any existing schedule
        _existing_sched = getattr(self, "water_change_schedule", {})
        _sched_text = ""
        if _existing_sched.get("enabled"):
            _when = _existing_sched.get("when", "")
            _tank = _existing_sched.get("tank", "")
            try:
                _dt = datetime.datetime.strptime(_when, "%Y-%m-%d %H:%M:%S")
                _sched_text = f"Scheduled: Tank {_tank}  |  {_dt.strftime('%A %H:%M')}"
            except Exception:
                _sched_text = f"Scheduled: Tank {_tank}  |  {_when}"
        status_label = tk.Label(
            container, text=_sched_text, font=("Arial", 11, "bold"),
            bg=getattr(self, "bg_color", None), fg="blue"
        )
        status_label.grid(row=4, column=0, columnspan=2, sticky="w", pady=(0, 10))

        btn_row = tk.Frame(container, bg=getattr(self, "bg_color", None))
        ...
        def _stop():
            sid = (tank_var.get() or "A").strip().upper()
            if not getattr(self, "water_change_active", {}).get(sid, False):
                return
            ok = self.show_confirm("Stop draining?", "Stop the drain pump now?", yes_text="Stop", no_text="Cancel")
            if not ok:
                return
            self.stop_water_change(sid, reason="manual")
            _close()

        def _submit():
            sid = (tank_var.get() or "A").strip().upper()
            if getattr(self, "water_change_active", {}).get(sid, False):
                return

            raw = (amount_var.get() or "").strip()
            try:
                remove_mmwg = float(raw)
            except Exception:
                self.show_error("Invalid value", "Please enter a numeric amount to remove (mmWG).")
                return
            if remove_mmwg <= 0:
                self.show_error("Invalid value", "Amount to remove must be greater than 0.")
                return

            if do_now_var.get():
                ok = self.show_confirm(
                    "Ready to start?",
                    "Are you ready to start?\n\n"
                    "By pressing 'Yes' the drain pump will activate.\n"
                    "Make sure you are ready for this action.",
                    yes_text="Yes",
                    no_text="No"
                )
                if not ok:
                ...
                target_min = int(min_var.get())

                if target_day == "Today":
                    dt = now.replace(hour=target_hour, minute=target_min, second=0, microsecond=0)
                    if dt < now:
                        self.show_error("Invalid Time", "Scheduled time for today has already passed.")
                        return
                elif target_day == "Tomorrow":
                    dt = (now + datetime.timedelta(days=1)).replace(hour=target_hour, minute=target_min, second=0, microsecond=0)
                else:
                    # Mon-Sun
                    ...
                    dt = (now + datetime.timedelta(days=days_ahead)).replace(hour=target_hour, minute=target_min, second=0, microsecond=0)

                self.water_change_schedule = {
                    "enabled": True,
                    "tank": sid,
                    "remove_mmwg": remove_mmwg,
                    "when": dt.strftime("%Y-%m-%d %H:%M:%S")
                }
                self.save_threshold_settings()
                status_label.config(text=f"Scheduled: Tank {sid}  |  {dt.strftime('%A %H:%M')}")
                _close()

        cancel_btn = tk.Button(btn_row, text="Cancel", command=_close)
        cancel_btn.grid(row=0, column=1, sticky="ew", padx=(0, 8)) # Submit on left, Cancel on right

        submit_btn = tk.Button(btn_row, text="Submit", command=_submit)
        submit_btn.grid(row=0, column=0, sticky="ew", padx=(0, 8))

        stop_btn = tk.Button(btn_row, text="STOP DRAIN", command=_stop)
        stop_btn.grid(row=0, column=2, sticky="ew")

        try:
            sid0 = (tank_var.get() or "A").strip().upper()
            if getattr(self, "water_change_active", {}).get(sid0, False):
                status_label.config(text=f"WATER CHANGE: Tank {sid0} DRAINING")
        except Exception:
            pass

        try:
            self.apply_theme(win)
        except Exception:
            pass

    def flash_water_change(self, label, key):
        """Flash a label like AUTO TOP UP ACTIVE, but for water-change draining."""
        def toggle_color():
            current_color = label.cget("fg")
            label.config(fg="red" if current_color == "green" else "green")
            self.flashing_labels[key] = label.after(500, toggle_color)

        # Cancel any existing flash before starting a new one
        self.stop_flashing(key)
        label.config(text="WATER CHANGE: DRAINING", fg="red")
        toggle_color()

    def start_water_change(self, sensor_id: str, remove_mmwg: float):
        """Begin draining the selected tank by remove_mmwg (mmWG)."""
        sid = (sensor_id or "A").strip().upper()
        if sid not in ("A", "B"):
            sid = "A"

        # Require a valid current reading to start
        current = None
        try:
            current = self.cached_readings.get(sid, {}).get("water_mmwg", None)
        ...
        if current is None:
            self.show_error("Cannot start", f"No valid water level reading for Tank {sid}.")
            return

        # Setup targets
        self.water_change_active[sid] = True
        self.water_change_start_mmwg[sid] = current
        self.water_change_target_mmwg[sid] = current - float(remove_mmwg)
        self.water_change_started_at[sid] = time.time()

        # Ensure top-up pump is OFF while draining
        ...
        try:
            ...
            if self.pump_states.get(pump_name, False):
                self.toggle_pump(pump_name, frame["pump_status"], frame["toggle_button"], force_state=False, suppress_auto_disable=True)
        except Exception:
            pass

        # Show flashing banner on the pump frame
        try:
            pf = self.pump_frame_a if sid == "A" else self.pump_frame_b
            self.flash_water_change(pf["auto_top_up_label"], key=f"WATER_CHANGE_{sid}")
        except Exception:
            pass

        # Activate drain relay
        try:
            GPIO.output(self.drain_gpio, GPIO.HIGH)
        except Exception as e:
            print(f"[ERROR] drain relay ON: {e}")

        print(f"[WATER CHANGE] START Tank {sid}: start={current:.2f} mmWG, target={self.water_change_target_mmwg[sid]:.2f} mmWG")
    ...
    def stop_water_change(self, sensor_id: str, reason: str = "complete"):
        ...
        # Stop flashing + clear banner text
        try:
            self.stop_flashing(f"WATER_CHANGE_{sid}")
        except Exception:
            pass
        try:
            pf = self.pump_frame_a if sid == "A" else self.pump_frame_b
            pf["auto_top_up_label"].config(text="", fg=getattr(self, "fg_color", "black"))
        except Exception:
            pass

        # Force an immediate re-evaluation of pumps after draining ends
        try:
            current = self.cached_readings.get(sid, {}).get("water_mmwg", None)
            if current is not None:
                self.safe_gui_update(lambda sid=sid, cur=float(current): self.control_pumps(sid, cur))
        except Exception:
            pass
    ...
    def water_change_tick(self, sensor_id: str, current_mmwg: float):
        ...
        IMPORTANT: Runs in a worker thread; any popups must be scheduled on the UI thread.
        """
        sid = (sensor_id or "A").strip().upper()
        if sid not in ("A", "B"):
            return
        ...
        try:
            ...
            if started and (time.time() - float(started)) > float(self.WATER_CHANGE_TIMEOUT_SECS):
                ...
                self.safe_gui_update(lambda sid=sid: self.show_info("Water change stopped", f"Tank {sid}: drain stopped due to timeout."))
                return
        except Exception:
            pass
        ...
        try:
            ...
            if float(current_mmwg) <= float(target):
                ...
                self.safe_gui_update(lambda sid=sid: self.show_info("Water change", f"Tank {sid}: drain complete. Auto top-up may now continue."))
        except Exception:
            pass
    ...
    def control_pumps(self, sensor_id, water_level_mmwg):
        ...
        if getattr(self, "water_change_active", {}).get(sensor_id, False):
            ...
            # IMPORTANT: start the flashing banner ONCE (don't restart every refresh),
            # so it flashes identically to AUTO TOP UP ACTIVE.
            key = f"WATER_CHANGE_{sensor_id}"
            try:
                if key not in getattr(self, "flashing_labels", {}):
                    self.flash_water_change(auto_top_up_label, key=key)
            except Exception:
                try:
    ...
    def open_settings_popup(self, sensor_id):
        ...
        _sc_row1 = tk.Frame(container)
        _sc_row1.pack(pady=(15, 10))
        tk.Button(_sc_row1, text="Submit", command=save_thresholds, width=15).pack(side="left", padx=5)
        tk.Button(_sc_row1, text="Cancel", command=popup.destroy, width=15).pack(side="left", padx=5)
        ...
        _nav_row1 = tk.Frame(container)
        _nav_row1.pack(pady=5)
        tk.Button(_nav_row1, text="Data Logging", command=lambda: (popup.destroy(), self.open_data_logging_popup()), width=15).pack(side="left", padx=5)
        tk.Button(_nav_row1, text="Graphics", command=lambda: (popup.destroy(), self.open_graphics_popup()), width=15).pack(side="left", padx=5)

        reboot_row = tk.Frame(container)
        reboot_row.pack(pady=10)
        ...
        _lockout_row = tk.Frame(container)
        _lockout_row.pack(pady=(10, 0))
        tk.Button(
            _lockout_row,
            text="Lockout",
            font=("Arial", 14, "bold"),
        ...
        tk.Button(
            ...
            text="Reset PIN",
            font=("Arial", 14, "bold"),
            command=self.open_reset_pin_popup
        ).pack(side="left")
                ...
                json.dump({
                    ...
                    "pump_auto_mode": {
                        ...
                        "B": bool(getattr(getattr(self, "pump_frame_b", {}) or {}, "get", lambda k, d=None: d)("auto_mode_var", tk.BooleanVar(value=False)).get()),
                                    ...
                                    "water_change_schedule": getattr(self, "water_change_schedule", {"enabled": False}),
}, f, indent=4)
        ...
        try:
            if os.path.exists(self._settings_path):
                with open(self._settings_path, "r") as f:
                    ...
                    self.water_change_schedule = data.get("water_change_schedule", {"enabled": False})
    ...
    def open_ro_settings_popup(self):
        ...
        _sc_row2 = tk.Frame(container)
        _sc_row2.pack(pady=(15, 10))
        tk.Button(_sc_row2, text="Submit", command=save_ro_alarm_settings, width=15).pack(side="left", padx=5)
        tk.Button(_sc_row2, text="Cancel", command=popup.destroy, width=15).pack(side="left", padx=5)
        tk.Button(
            container,
           text="Reset Sensor",
            state=tk.NORMAL if self.sensors.get(sensor_id, {}).get("is_running") else tk.DISABLED,
    ...
    def open_ph_settings_popup(self):
        ...
        _sc_row3 = tk.Frame(container)
        _sc_row3.pack(pady=(15, 10))
        tk.Button(_sc_row3, text="Submit", command=save_ph_settings, width=15).pack(side="left", padx=5)
        tk.Button(_sc_row3, text="Cancel", command=popup.destroy, width=15).pack(side="left", padx=5)
        tk.Button(
            container,
            text="Reset Sensor",
            state=tk.NORMAL if self.sensors.get(sensor_id, {}).get("is_running") else tk.DISABLED,
    ...
    def open_tds_settings_popup(self):
        ...
        _sc_row4 = tk.Frame(container)
        _sc_row4.pack(pady=(15, 10))
        tk.Button(_sc_row4, text="Submit", command=save_tds_settings, width=15).pack(side="left", padx=5)
        tk.Button(_sc_row4, text="Cancel", command=popup.destroy, width=15).pack(side="left", padx=5)
        tk.Button(
            container,
            text="Reset Sensor",
            state=tk.NORMAL if self.sensors.get(sensor_id, {}).get("is_running") else tk.DISABLED,
            ...
            def style_widget(w):
                try:
                    ...
                    if wtype in ["Frame", "LabelFrame", "Toplevel"]:
                        ...
                        # Preserve red text for Exit & Restart / Reboot System buttons
                        try:
                            current_fg = str(w.cget("fg")).lower()
                        except Exception:
                            current_fg = ""
                        # Robust check for red to prevent dark mode override
                        final_fg = "red" if ("red" in current_fg or current_fg == "#ff0000" or current_fg == "red") else fg
                        w.configure(
                            bg=bg,
                            fg=final_fg,
                            activebackground=bg,
                            activeforeground=final_fg,
                            relief=tk.RAISED,
                            highlightthickness=0,
                            borderwidth=2
                        )
    ...
    def restart_program(self):
        """
        Cleanly shut down sensors, pumps and GPIO, then restart the Python process.
        Called by the 'Exit & Restart' buttons in settings screens.
        """
        if not self.show_confirm(
            "Restart Application",
            "Are you sure you want to Exit & Restart the application?\n\n"
            "All pumps and relays will be turned OFF and the program will reload.",
            yes_text="Restart",
            no_text="Cancel",
        ):
            return

        # 1) Stop sensor threads and close ports
        try:
            for sensor_id, sensor in self.sensors.items():
                ...
                try:
                    ...
                    print(f"[RESTART] Could not close port for Sensor {sensor_id}: {e}")
        except Exception as e:
            print(f"[RESTART] Error while stopping sensors: {e}")

        # 2) Turn OFF pumps and relays safely
        try:
            for pump_name, pin in self.pump_gpio.items():
                try:
                    GPIO.output(pin, GPIO.LOW)
                except Exception as e:
                    print(f"[RESTART] Could not turn off pump '{pump_name}': {e}")
            try:
                GPIO.output(self.relay4_gpio, GPIO.LOW)
            except Exception as e:
                print(f"[RESTART] Could not turn off relay4: {e}")
            try:
                GPIO.output(self.drain_gpio, GPIO.LOW)
            except Exception as e:
                print(f"[RESTART] Could not turn off drain relay: {e}")
            GPIO.cleanup()
        except Exception as e:
            print(f"[RESTART] GPIO cleanup failed: {e}")

        # 3) Exec-restart the current script
        try:
            python = sys.executable or "python3"
            os.execv(python, [python] + sys.argv)
        except Exception as e:
            print(f"[RESTART] execv failed: {e}")
            self.root.destroy()


    def reboot_system(self):
        """Cleanly shut down I/O then reboot the entire Raspberry Pi."""
        if not self.show_confirm(
            "Reboot System",
            "Are you sure you want to reboot the Raspberry Pi?\n\nAll pumps and relays will be turned OFF.",
            yes_text="Reboot",
            no_text="Cancel",
        ):
            return

        # Stop sensors / close ports (best-effort)
        try:
            for sensor_id, sensor in self.sensors.items():
                sensor["is_running"] = False
                port = sensor.get("port")
                try:
                    ...
                    print(f"[REBOOT] Could not close port for Sensor {sensor_id}: {e}")
        except Exception as e:
            print(f"[REBOOT] Error while stopping sensors: {e}")

        # Turn OFF pumps and relays safely
        try:
            for pump_name, pin in self.pump_gpio.items():
                try:
                    GPIO.output(pin, GPIO.LOW)
                except Exception as e:
                    print(f"[REBOOT] Could not turn off pump '{pump_name}': {e}")
            try:
                GPIO.output(self.relay4_gpio, GPIO.LOW)
            except Exception as e:
                print(f"[REBOOT] Could not turn off relay4: {e}")
            try:
                GPIO.output(self.drain_gpio, GPIO.LOW)
            except Exception as e:
                print(f"[REBOOT] Could not turn off drain relay: {e}")
            try:
                GPIO.cleanup()
            except Exception as e:
                print(f"[REBOOT] GPIO cleanup failed: {e}")
        except Exception as e:
            print(f"[REBOOT] Error during IO shutdown: {e}")

        # Reboot the OS
        try:
            # Most deployments already run this app with sudo/root for GPIO.
            subprocess.Popen(["sudo", "reboot"])
        except Exception as e:
            print(f"[REBOOT] Failed to invoke reboot: {e}")
    ...
    def _close_all_popups(self):
        """Close all Toplevel popups so the main UI is visible."""
        try:
            import tkinter as tk
        except ImportError:
            return
        # Destroy any Toplevel child windows
    ...
    # ---------------- Lockout / PIN ----------------
    def _lockout_config_path(self):
        try:
            base = os.path.dirname(os.path.abspath(__file__))
        except Exception:
            base = os.getcwd()
        return os.path.join(base, "lockout.json")

    def _load_lockout_config(self):
        path = self._lockout_config_path()
        try:
            ...
            print(f"[LOCKOUT] Could not read {path}: {e}")
            return {}

    def _save_lockout_config(self, cfg):
        path = self._lockout_config_path()
        try:
            ...
            print(f"[LOCKOUT] Could not write {path}: {e}")
            return False

    def _hash_pin(self, pin, salt_b64=None, iterations=200_000):
        if salt_b64:
        ...
        return salt_b64, iterations, hash_b64

    def _verify_pin(self, pin, cfg):
        try:
            salt_b64 = cfg.get("salt")
            iterations = int(cfg.get("iterations", 200_000))
            stored = cfg.get("pin_hash")
            if not salt_b64 or not stored: return False
            _, _, computed = self._hash_pin(pin, salt_b64=salt_b64, iterations=iterations)
            return secrets.compare_digest(computed, stored)
        except Exception: return False

    def _pin_keypad_prompt(self, title, message, allow_cancel=True, min_len=4, max_len=6, timeout=None):
        import tkinter as tk
        popup = tk.Toplevel(self.root)
        popup.title(title)
        popup.transient(self.root)

        # Set a larger fixed size to ensure all buttons are visible
        ...
        def _ok():
            ...
            if min_len <= len(s) <= max_len:
                ...
                status_var.set(f"Enter {min_len}-{max_len} digits")
        ...
        tk.Label(container, text=title, font=("Arial", 18, "bold")).pack(pady=(0, 10))
        tk.Label(container, text=message, wraplength=450, font=("Arial", 12)).pack(pady=(0, 10))

        pin_var = tk.StringVar()
        display = tk.Label(container, textvariable=pin_var, font=("Arial", 24, "bold"), bg="white", relief="sunken", width=10)
        display.pack(pady=10)

        status_var = tk.StringVar(value=f"Required: {min_len}-{max_len} digits")
        tk.Label(container, textvariable=status_var, fg="red").pack()

        keypad_frame = tk.Frame(container)
        keypad_frame.pack(pady=10)

        def _add(digit):
            if len(pin_var.get()) < max_len:
                pin_var.set(pin_var.get() + str(digit))
        ...
        for (text, r, c) in buttons:
            if text.isdigit():
                cmd = lambda t=text: _add(t)
            elif text == '<':
                cmd = _back
            else:
                cmd = lambda: pin_var.set("")
            tk.Button(keypad_frame, text=text, width=5, height=2, font=("Arial", 14, "bold"), command=cmd).grid(row=r, column=c, padx=5, pady=5)

        btn_frame = tk.Frame(container)
        btn_frame.pack(side="bottom", fill="x", pady=10)

        tk.Button(btn_frame, text="OK", width=10, bg="green", fg="white", font=("Arial", 12, "bold"), command=_ok).pack(side="left", expand=True)
        if allow_cancel:
            tk.Button(btn_frame, text="Cancel", width=10, bg="red", fg="white", font=("Arial", 12, "bold"), command=_cancel).pack(side="right", expand=True)
    ...
    def _ensure_lockout_pin(self):
        cfg = self._load_lockout_config()
        if cfg.get("pin_hash"): return True
        if not self.show_confirm("Lockout Setup", "No lockout PIN is set. Set one now?", yes_text="Set PIN", no_text="Cancel"):
            return False
        pin1 = self._pin_keypad_prompt("Set Lockout PIN", "Enter a new 4–6 digit PIN:", allow_cancel=True)
        if not pin1: return False
        pin2 = self._pin_keypad_prompt("Confirm PIN", "Re-enter the PIN to confirm:", allow_cancel=True)
        if not pin2 or pin1 != pin2:
            if pin1 != pin2: self.show_confirm("Error", "PINs did not match.", yes_text="OK", no_text="")
            return False
        salt_b64, iterations, pin_hash_b64 = self._hash_pin(pin1)
        return self._save_lockout_config({"pin_hash": pin_hash_b64, "salt": salt_b64, "iterations": iterations, "min_len": 4, "max_len": 6})

    def _ensure_lockout_pin(self):
        cfg = self._load_lockout_config()
        pin_hash = (cfg.get("pin_hash") or "").strip() if isinstance(cfg.get("pin_hash"), str) else cfg.get("pin_hash")
        if pin_hash:
            return True

        if not self.show_confirm(
            "Lockout Setup",
            "No lockout PIN is set. Would you like to set one now?\n\nPIN must be 4–6 digits (numbers only).",
            yes_text="Set PIN",
            no_text="Cancel",
        ):
            return False
        ...
        if not pin2:
            return False
        if pin1 != pin2:
            self.show_confirm("PIN Mismatch", "PINs did not match. Please try again.", yes_text="OK", no_text="")
            return False
        ...
        return self._save_lockout_config(cfg)

    
    def open_reset_pin_popup(self):
        """Themed popup to reset the system PIN (New PIN + Confirm PIN)."""
        # Step 1: Get New PIN
        new_pin = self._pin_keypad_prompt("Enter New PIN", "Please enter your new 4-digit PIN:")
        if not new_pin:
            return

        if len(new_pin) != 4 or not new_pin.isdigit():
            self.show_error("Invalid PIN", "PIN must be exactly 4 digits.")
            return

        # Step 2: Confirm New PIN
        confirm_pin = self._pin_keypad_prompt("Confirm New PIN", "Please re-enter your new PIN to confirm:")
        if not confirm_pin:
            return

        if new_pin != confirm_pin:
            self.show_error("Mismatch", "PINs do not match. Reset aborted.")
            return

        # Step 3: Save
        try:
            self.visual_settings["pin"] = new_pin
            self.save_threshold_settings()
            self.show_info("Success", "System PIN has been updated successfully.")
        except Exception as e:
            self.show_error("Error", f"Failed to save PIN: {e}")

    def lockout(self):
        """
        Lock the UI: close all popups, show main dashboard, place an invisible
        Frame over the root window to intercept all clicks. Tap anywhere to
        bring up the PIN keypad (10s timeout). No Toplevel - no black box.
        """
        if not self._ensure_lockout_pin():
            return

        # 1) Close all open popups so the main dashboard is visible
        self._close_all_popups()

        # 2) Place an invisible Frame over the entire root window
        #    relwidth/relheight=1 means it covers 100% of the root window
        lock_frame = tk.Frame(self.root, bg="", cursor="hand2")
        lock_frame.place(relx=0, rely=0, relwidth=1, relheight=1)
        lock_frame.lift()  # Ensure it's on top of all other widgets

        def _on_tap(event=None):
            # Temporarily hide the frame so keypad can be interacted with
            lock_frame.place_forget()

            pin = self._pin_keypad_prompt(
                "System Locked",
                "Enter PIN to unlock:",
                allow_cancel=True,
                timeout=10,
            ...
            if pin and self._verify_pin(pin, self._load_lockout_config()):
                # Correct PIN - destroy the lock frame and unlock
                lock_frame.destroy()
                return

            # Wrong / cancelled / timeout - put the lock frame back
            lock_frame.place(relx=0, rely=0, relwidth=1, relheight=1)
            lock_frame.lift()

        lock_frame.bind("<Button-1>", _on_tap)
