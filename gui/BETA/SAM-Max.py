...
# Auto GitHub Update
    ...
    def open_water_change_popup(self, default_sensor="A"):
        ...
        status_label = tk.Label(
            container, text="", font=("Arial", 11, "bold"),
            bg=getattr(self, "bg_color", None), fg="orange"
        )
        ...
        def _submit():
            ...
            if do_now_var.get():
                ...
                self.save_threshold_settings()
                self.show_info("Scheduled", f"Water change scheduled for Tank {sid} on {dt.strftime('%a %H:%M')}")
                _close()
    ...
    def open_settings_popup(self, sensor_id):
        ...
        sc_row = tk.Frame(container)
        sc_row.pack(pady=(15, 5))
        tk.Button(sc_row, text="Submit", command=save_thresholds).pack(side="left", padx=(0, 10))
        tk.Button(sc_row, text="Cancel", command=popup.destroy).pack(side="left")
        ...
        dg_row = tk.Frame(container)
        dg_row.pack(pady=10)
        tk.Button(dg_row, text="Data Logging", command=lambda: (popup.destroy(), self.open_data_logging_popup())).pack(side="left", padx=(0, 10))
        tk.Button(dg_row, text="Graphics", command=lambda: (popup.destroy(), self.open_graphics_popup())).pack(side="left")
    ...
    def open_ro_settings_popup(self):
        ...
        sc_row = tk.Frame(container)
        sc_row.pack(pady=(15, 5))
        tk.Button(sc_row, text="Submit", command=save_ro_alarm_settings).pack(side="left", padx=(0, 10))
        tk.Button(sc_row, text="Cancel", command=popup.destroy).pack(side="left")
        tk.Button(
            container,
           text="Reset Sensor",
    ...
    def open_ph_settings_popup(self):
        ...
        sc_row = tk.Frame(container)
        sc_row.pack(pady=(15, 5))
        tk.Button(sc_row, text="Submit", command=save_ph_settings).pack(side="left", padx=(0, 10))
        tk.Button(sc_row, text="Cancel", command=popup.destroy).pack(side="left")
        tk.Button(
            container,
            text="Reset Sensor",
    ...
    def open_tds_settings_popup(self):
        ...
        sc_row = tk.Frame(container)
        sc_row.pack(pady=(15, 5))
        tk.Button(sc_row, text="Submit", command=save_tds_settings).pack(side="left", padx=(0, 10))
        tk.Button(sc_row, text="Cancel", command=popup.destroy).pack(side="left")
        tk.Button(
            container,
            text="Reset Sensor",
    ...
    def _pin_keypad_prompt(self, title, message, allow_cancel=True, min_len=4, max_len=6, timeout=None):
        ...
        def _reset_timer():
            if timeout:
                if timer_id[0]: popup.after_cancel(timer_id[0])
                timer_id[0] = popup.after(timeout * 1000, _cancel)

        _reset_timer()

        container = tk.Frame(popup, padx=24, pady=18)
        container.pack(fill="both", expand=True)
        tk.Label(container, text=title, font=("Arial", 18, "bold")).pack(anchor="w", pady=(0, 8))
        tk.Label(container, text=message, justify="left", wraplength=700, font=("Arial", 14)).pack(anchor="w", pady=(0, 12))

        pin_var = tk.StringVar(value="")
        masked_var = tk.StringVar(value="")
        display = tk.Label(container, textvariable=masked_var, font=("Arial", 28, "bold"), anchor="w")
        display.pack(fill="x", pady=(0, 12))

        status_var = tk.StringVar(value=f"Enter {min_len}-{max_len} digits")
        status = tk.Label(container, textvariable=status_var, font=("Arial", 12))
        status.pack(anchor="w", pady=(0, 10))

        keypad = tk.Frame(container)
        keypad.pack()
        ...
        def _ok():
            ...
            if min_len <= len(s) <= max_len:
                ...
                status_var.set(f"PIN must be {min_len}-{max_len} digits")
                _reset_timer()
        ...
        for txt, r, c in buttons:
            if txt.isdigit(): cmd = lambda t=txt: _press(t)
            elif txt == "⌫": cmd = _back
            else: cmd = _clear
            tk.Button(keypad, text=txt, width=8, height=2, font=("Arial", 16, "bold"), command=cmd).grid(row=r, column=c, padx=8, pady=8)

        action_row = tk.Frame(container)
        action_row.pack(pady=(18, 0), fill="x")
        if allow_cancel:
            tk.Button(action_row, text="Cancel", width=12, font=("Arial", 14), command=_cancel).pack(side="right", padx=(10, 0))
        tk.Button(action_row, text="OK", width=12, font=("Arial", 14, "bold"), command=_ok).pack(side="right")
