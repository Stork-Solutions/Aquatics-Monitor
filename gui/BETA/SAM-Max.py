...
class SensorGUI:
    ...
    def open_settings_popup(self, sensor_id):
        ...
        tk.Checkbutton(container, text="Enable Temperature Alarm", variable=temp_alarm_var).pack(pady=(12, 2))
...
# Persist connection choice (fixed port 8888)
    ...
    def check_temp_alarm(self, sensor_id, temp_reading, frame):
        ...
        - Uses the SAME 'connection_status' label area for flashing text (APPROACHING LIMIT / LEVEL CRITICAL).
        - Uses the same sound triggers (approaching/critical).
        Settings keys (per sensor): temp_alarm_enabled, temp_min, temp_max, use_fahrenheit
        """
        try:
            settings = self.display_units.get(sensor_id, {}) or {}

            # Use the status label (same place as pH/TDS / RO alarms)
            label = frame.get("connection_status")
            if not label:
                return

            alarm_key = f"temp_{sensor_id}"

            # Disabled => stop effects and return to normal (do not force text if sensor is disconnected)
            if not settings.get("temp_alarm_enabled", False):
                try:
                    self._set_alarm_state(alarm_key, "normal", label)
                except Exception:
                    pass
                return
            ...
            # Buffer near limits (matches pH/TDS style, scaled for °F)
            margin = 1.0 if use_f else 0.5

            if val <= lo or val >= hi:
                self._set_alarm_state(alarm_key, "critical", label)
            elif (lo < val <= lo + margin) or (hi - margin <= val < hi):
                self._set_alarm_state(alarm_key, "approaching", label)
            else:
                self._set_alarm_state(alarm_key, "normal", label)
            ...
            try:
                label = frame.get("connection_status")
                if label:
                    self._set_alarm_state(f"temp_{sensor_id}", "normal", label)
            except Exception:
                pass

    def start_alarm_flashdef start_alarm_flash(self, label, sensor_key, base_color):
