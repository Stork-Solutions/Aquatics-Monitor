...
class SensorGUI:
    ...
    def open_manage_logs_popup(self):
        ...
        def do_email():
            ...
            if not fname:
                ...
                return
            if not to_addr:
                self.show_error("No email", "No recipient email address entered.")
                return
            src = os.path.join(self.logs_dir, fname)
            try:
                # Save recipient as default
                self.email_settings["to_email"] = to_addr
                self.save_email_settings()
                self._send_log_email(src, to_addr)
    ...
    def open_ro_settings_popup(self):
        ...
        tmin_label = tk.Label(container, text="Min Temperature:")
        tmin_label.pack()
        tmin_entry = tk.Entry(container, textvariable=temp_min_var)
        tmin_entry.pack()

        tmax_label = tk.Label(container, text="Max Temperature:")
        tmax_label.pack()
        tmax_entry = tk.Entry(container, textvariable=temp_max_var)
        tmax_entry.pack()
        ...
        while self.sensors.get(sensor_id, {}).get("is_running", False):
            try:
                ...
                if sensor_id == "A":
                    ...
                    # Temperature alarm (pH sensor)
                    try:
                        self.check_temp_alarm("D", temperature, self.ph_level_frame)
                    except Exception:
                        pass
                    if ph_level:
                    ...
                    if tds_level:
                        try:
                            self.check_tds_alarm("E", tds_level)
                        except Exception:
                            pass
                    if temperature:
                        try:
                            self.check_temp_alarm("E", temperature, self.tds_level_frame)
                        except Exception:
                            pass
    ...
    def check_temp_alarm(self, sensor_id, temp_reading, frame):
        """
        Temperature alarm that mimics the pH/TDS alarm behaviour (flashing text + alarm WAV).
        Uses display_units[sensor_id] keys: temp_alarm_enabled, temp_min, temp_max, use_fahrenheit.
        Flashes the *temperature label* to avoid clashing with existing status alarms (RO / pH / TDS).
        """
        try:
            settings = self.display_units.get(sensor_id, {}) or {}
            label = frame.get("temperature_label")
            ...
            if not settings.get("temp_alarm_enabled", False):
                # Disabled => stop flashing immediately
                try:
                    self.stop_alarm_flash(alarm_key, restore=False, label=label)
                except Exception:
                    pass
                self.alarm_state[alarm_key] = "normal"
                return

            val_c = self._num(temp_reading)
            if val_c is None:
                # No valid reading => do not alarm
                try:
                    self.stop_alarm_flash(alarm_key, restore=False, label=label)
                except Exception:
                    pass
            ...
            if lo is None or hi is None or lo >= hi:
                # Misconfigured => treat as disabled
                try:
                    self.stop_alarm_flash(alarm_key, restore=False, label=label)
                except Exception:
                    pass
            ...
            # Near-threshold buffer (mimics existing alarm behaviour)
            ...
            prev = self.alarm_state.get(alarm_key, "normal")
            if prev == state:
                return

            # Stop any old flashing for this temp alarm
            try:
                self.stop_alarm_flash(alarm_key, restore=False, label=label)
            except Exception:
                pass

            self.alarm_state[alarm_key] = state

            if state == "normal":
                return

            # Preserve the temperature reading in the text, but use the same alarm phrases
            unit = "°F" if use_f else "°C"
            try:
                if state == "approaching":
                    label.config(text=f"Temperature: {val:.1f} {unit}  (APPROACHING LIMIT)")
                    self.start_alarm_flash(label, alarm_key, base_color="orange")
                    self._maybe_play_alarm(alarm_key, "approaching")
                else:
                    label.config(text=f"Temperature: {val:.1f} {unit}  (LEVEL CRITICAL)")
                    self.start_alarm_flash(label, alarm_key, base_color="red")
                    self._maybe_play_alarm(alarm_key, "critical")
            except Exception:
                pass
    ...
    def _maybe_play_alarm(self, sensor_id, state, min_interval=2.0):
        # Accept legacy "approaching" state too
        state_norm = "approaching" if state in ("approaching", "approaching") else ("critical" if state == "critical" else state)
        key = "approaching" if state_norm == "approaching" else "critical"
        now = _time.monotonic()
        k = (sensor_id, state_norm)
    ...
    def _set_alarm_state(self, sensor_id, new_state, label):
        """
        new_state: "normal" | "approaching" | "critical" (accepts legacy "approaching").
        Mimics existing pH/TDS alarm behaviour: sets status text + flashes + plays sound (edge-triggered).
        """
        try:
            # Normalize legacy state name
            ...
            if new_state == "approaching":
                try:
                    ...
                    pass
                self.start_alarm_flash(label, sensor_id, base_color="orange")
                self._maybe_play_alarm(sensor_id, "approaching")
                return
            ...
            try:
                ...
                pass
            self.start_alarm_flash(label, sensor_id, base_color="red")
            self._maybe_play_alarm(sensor_id, "critical")

        except Exception as e:
            print(f"[ALARM] _set_alarm_state error ({sensor_id}): {e}")
