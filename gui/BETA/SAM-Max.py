...
# Auto GitHub Update
    ...
    def open_water_change_popup(self, default_sensor="A"):
        """Popup to start/monitor an automated drain (water change)."""
        ...
        # Keep consistent behaviour with other popups
        try:
        ...
        status_label.grid(row=2, column=0, columnspan=2, sticky="w", pady=(0, 10))
        ...
        def _submit():
            sid = (tank_var.get() or "A").strip().upper()
            # If already draining, don't allow a new start
        ...
        submit_btn.grid(row=0, column=1, sticky="ew", padx=(0, 8))

        stop_btn = tk.Button(btn_row, text="STOP DRAIN", command=_stop)
        stop_btn.grid(row=0, column=2, sticky="ew")

        # If a water change is already active for this default tank, show status and enable stop.
        try:
