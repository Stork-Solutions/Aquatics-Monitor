...
# --- Embedded UI icon (water drop) ---
WATER_DROP_ICON_PNG_B64 = (
...
class SensorGUI:
    ...
    def attach_water_change_button(self, frame, command):
        ...
        Note: The 💧 emoji often fails to render on Raspberry Pi builds depending on fonts.
        This uses an embedded PNG icon so it is consistent across devices.
        """
        try:
            if not hasattr(self, "_water_drop_img") or self._water_drop_img is None:
                # tk.PhotoImage supports base64 PNG data via the 'data' parameter.
                self._water_drop_img = tk.PhotoImage(data=WATER_DROP_ICON_PNG_B64)
        except Exception:
            self._water_drop_img = None

        if self._water_drop_img is not None:
            btn = tk.Button(
                frame,
                image=self._water_drop_img,
                command=command,
                bd=0,
                relief="flat",
                highlightthickness=0
            )
            # Keep a per-button reference too (extra-safe against GC quirks)
            btn.image = self._water_drop_img
        else:
        ...
        # Place close to the right edge, aligned like the settings cog button.
        # If the pump frame already has a top-right overlay control, this sits just to its left.
        btn.place(relx=1.0, rely=0.0, anchor="ne", x=-42, y=6)
        return btn
    # Auto GitHub Update
        ...
        try:
            ...
            def _stop():
                ...
                if not getattr(self, "water_change_active", {}).get(sid, False):
                    return
                ok = messagebox.askyesno("Stop draining?", "Stop the drain pump now?")
                if not ok:
                    return
            ...
            def _submit():
                ...
                try:
                    ...
                    messagebox.showerror("Invalid value", "Please enter a numeric amount to remove (mmWG).")
                    return
                if remove_mmwg <= 0:
                    messagebox.showerror("Invalid value", "Amount to remove must be greater than 0.")
                    return
            ...
            cancel_btn = tk.Button(btn_row, text="Cancel", font=("Arial", 12), command=_close)
            cancel_btn.grid(row=0, column=0, sticky="ew", padx=(0, 6))

            stop_btn = tk.Button(btn_row, text="STOP DRAIN", font=("Arial", 12, "bold"),
                                 command=_stop)
            stop_btn.grid(row=0, column=1, sticky="ew", padx=(6, 6))

            submit_btn = tk.Button(btn_row, text="Submit", font=("Arial", 12, "bold"), command=_submit)
            submit_btn.grid(row=0, column=2, sticky="ew", padx=(6, 0))

            def _refresh_state(*_):
                sid = (tank_var.get() or "A").strip().upper()
                active = getattr(self, "water_change_active", {}).get(sid, False)

                if active:
                    # Show status + enable stop, disable submit / editing
                    try:
                        start = self.water_change_start_mmwg.get(sid, None)
                        target = self.water_change_target_mmwg.get(sid, None)
                        if start is not None and target is not None:
                            status_label.config(text=f"Status: WATER CHANGE DRAINING (target {target:.1f} mmWG)")
                        else:
                            status_label.config(text="Status: WATER CHANGE DRAINING")
                    except Exception:
                        status_label.config(text="Status: WATER CHANGE DRAINING")
            ...
            # Update UI when tank selection changes
            try:
                tank_var.trace_add("write", _refresh_state)
            except Exception:
                pass
            _refresh_state()
            ...
            print(f"[ERROR] open_water_change_popup: {e}")
    ...
    def start_water_change(self, sensor_id: str, remove_mmwg: float):
        ...
        if current is None:
            messagebox.showerror("Cannot start", f"No valid water level reading for Tank {sid}.")
            return
    ...
    def stop_water_change(self, sensor_id: str, reason: str = "complete"):
        ...
        try:
            ...
            pf["auto_top_up_label"].config(text="", fg="black")
        except Exception:
            pass
    ...
    def water_change_tick(self, sensor_id: str, current_mmwg: float):
        ...
        try:
            ...
            if started and (time.time() - float(started)) > float(self.WATER_CHANGE_TIMEOUT_SECS):
                ...
                try:
                    messagebox.showwarning("Water change stopped", f"Tank {sid}: drain stopped due to timeout.")
                except Exception:
                    pass
                return
        except Exception:
        ...
        try:
            ...
            if float(current_mmwg) <= float(target):
                ...
                try:
                    messagebox.showinfo("Water change", f"Tank {sid}: drain complete. Auto top-up may now continue.")
                except Exception:
                    pass
        except Exception:
            pass
