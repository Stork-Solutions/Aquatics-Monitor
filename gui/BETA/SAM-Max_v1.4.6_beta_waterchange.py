...
class SensorGUI:
    def __init__(self, root):
        ...
        # Drain Pump (Auto Water Change) uses Relay 3 on GPIO 22
        self.drain_gpio = 22
        ...
        # ===== Auto Water Change (v1.4.6) =====
        # Uses drain relay on GPIO 22 (self.drain_gpio)
        self.water_change_active = False
        self.water_change_target = None   # "A" or "B"
        self.water_change_start_mmwg = None
        self.water_change_stop_mmwg = None
        self.water_change_started_at = None
        self.WATER_CHANGE_MAX_SECS = 30 * 60  # safety timeout
        self._water_change_job = None
    ...
    # ===== Auto Water Change (v1.4.6) =====
    def open_water_change_popup(self, default_sensor=None):
        """Popup to start an automated water change (drain only)."""
        popup = tk.Toplevel(self.root)
        popup.title("Auto Water Change")
        popup.transient(self.root)
        try:
            popup.attributes("-fullscreen", True)
        except Exception:
        ...
        tk.Label(container, text="Auto Water Change", font=("Arial", 16, "bold")).pack(anchor="w", pady=(0, 12))

        form = tk.Frame(container)
        form.pack(fill="x", pady=(0, 12))

        tk.Label(form, text="Select tank:", font=("Arial", 12, "bold")).grid(row=0, column=0, sticky="w", padx=(0, 10), pady=6)
        tank_var = tk.StringVar(value=(default_sensor or "A"))
        tank_dd = ttk.Combobox(form, textvariable=tank_var, values=["A", "B"], state="readonly", width=10)
        tank_dd.grid(row=0, column=1, sticky="w", pady=6)

        tk.Label(form, text="Amount to remove (mmWG):", font=("Arial", 12, "bold")).grid(row=1, column=0, sticky="w", padx=(0, 10), pady=6)
        amount_var = tk.StringVar(value="")
        amount_entry = tk.Entry(form, textvariable=amount_var, width=12)
        amount_entry.grid(row=1, column=1, sticky="w", pady=6)
        ...
        def _submit():
            sid = (tank_var.get() or "").strip().upper()
            amt = safe_parse_float(amount_var.get())
            if sid not in ("A", "B"):
                self.show_error("Auto Water Change", "Please select a valid tank (A or B).")
                return
            if amt is None or amt <= 0:
                self.show_error("Auto Water Change", "Please enter a valid amount to remove (mmWG).")
                return
            if self.water_change_active:
                self.show_error("Auto Water Change", "A water change is already in progress.")
                return
            ...
            try:
                ...
                pass
            self.start_water_change(sensor_id=sid, amount_mmwg=float(amt))

        tk.Button(btns, text="Cancel", command=_cancel, width=10).pack(side="right", padx=(8, 0))
        tk.Button(btns, text="Submit", command=_submit, width=10).pack(side="right")
    ...
    def start_water_change(self, sensor_id: str, amount_mmwg: float):
        """Activate drain pump until `amount_mmwg` has been removed from the selected tank."""
        sensor_id = (sensor_id or "").strip().upper()
        if sensor_id not in ("A", "B"):
            self.show_error("Auto Water Change", "Invalid tank selection.")
            return

        # Must have a live reading to compute the stop point
        start_mmwg = None
        try:
            start_mmwg = self.cached_readings.get(sensor_id, {}).get("water_mmwg", None)
            if start_mmwg is None:
                # try parsing from UI string if needed (fallback)
                start_mmwg = safe_parse_float(start_mmwg)
        except Exception:
            start_mmwg = None

        if start_mmwg is None:
            self.show_error("Auto Water Change", f"No water level reading available for Tank {sensor_id}.")
            return

        try:
            start_mmwg = float(start_mmwg)
        except Exception:
            self.show_error("Auto Water Change", f"Invalid water level reading for Tank {sensor_id}.")
            return

        stop_mmwg = start_mmwg - float(amount_mmwg)
        # Clamp to 0 to avoid negative targets
        if stop_mmwg < 0:
            stop_mmwg = 0.0

        # Ensure BOTH top-up pumps are OFF while draining (hold auto)
        try:
            if self.pump_states.get("RO Pump A", False):
                self.toggle_pump("RO Pump A", self.pump_frame_a["pump_status"], self.pump_frame_a["toggle_button"], force_state=False, suppress_auto_disable=True)
            if self.pump_states.get("RO Pump B", False):
                self.toggle_pump("RO Pump B", self.pump_frame_b["pump_status"], self.pump_frame_b["toggle_button"], force_state=False, suppress_auto_disable=True)
        except Exception:
            pass

        # Arm state
        self.water_change_active = True
        self.water_change_target = sensor_id
        self.water_change_start_mmwg = start_mmwg
        self.water_change_stop_mmwg = stop_mmwg
        self.water_change_started_at = time.time()

        print(f"[WATER CHANGE] Start Tank {sensor_id}: start={start_mmwg:.2f} mmWG, stop={stop_mmwg:.2f} mmWG")
        ...
        try:
            ...
            self.water_change_active = False
            self.show_error("Auto Water Change", f"Failed to start drain relay: {e}")
            return

        # Tick loop (UI thread)
        self._schedule_water_change_tick()

    def _schedule_water_change_tick(self):
        try:
            if self._water_change_job:
                self.root.after_cancel(self._water_change_job)
        except Exception:
            pass
        self._water_change_job = self.root.after(500, self._water_change_tick)

    def _stop_water_change(self, *, show_message=None, is_error=False):
        """Stop drain relay and clear water-change state."""
        try:
            GPIO.output(self.drain_gpio, GPIO.LOW)
        except Exception:
            pass
        ...
        if show_message:
            if is_error:
                self.show_error("Auto Water Change", show_message)
            else:
                self.show_info("Auto Water Change", show_message)

    def _water_change_tick(self):
        if not self.water_change_active or self.water_change_target not in ("A", "B"):
            return

        # Safety timeout
        try:
            if self.water_change_started_at and (time.time() - self.water_change_started_at) > self.WATER_CHANGE_MAX_SECS:
                self._stop_water_change(show_message="Safety timeout reached. Drain stopped.", is_error=True)
                return
        except Exception:
            pass

        sid = self.water_change_target
        stop_mmwg = self.water_change_stop_mmwg
        ...
        if current is None:
            # If we lose readings briefly, keep trying for a short while (handled by timeout)
            self._schedule_water_change_tick()
            return
        ...
        # Stop condition
        if stop_mmwg is not None and current <= float(stop_mmwg):
            print(f"[WATER CHANGE] Complete Tank {sid}: current={current:.2f} mmWG (<= {float(stop_mmwg):.2f})")
            self._stop_water_change(show_message="Drain complete. Auto top-up will resume.")
            return
    ...
    def attach_water_change_button(self, frame, command):
        """Attach a 💧 water-change button at the top-right of a pump frame (overlay)."""
        btn = tk.Button(
            frame,
            text="💧",
            font=("Arial", getattr(self, "_s", lambda x: x)(12)),
    ...
    def create_pump_frame(self, title, row, column):
        ...
        # Water Change button (top-right)
        # Default tank selection based on pump title (A/B), but user can change in popup.
        default_sid = "A" if title.endswith("A") else ("B" if title.endswith("B") else None)
    ...
    def control_pumps(self, sensor_id, water_level_mmwg):
        ...
        # If an Auto Water Change is running for this tank, HOLD auto top-up logic.
        # We do not uncheck Auto Mode; we simply pause control until draining completes.
        if getattr(self, "water_change_active", False) and getattr(self, "water_change_target", None) == sensor_id:
            return
