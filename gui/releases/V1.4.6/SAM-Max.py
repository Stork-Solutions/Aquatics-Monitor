...
# Auto GitHub Update
    ...
    def schedule_water_change_tick(self):
        """Background loop to check for scheduled water changes every 60 seconds."""
        try:
                        ...
                        if now >= scheduled_time:
                            ...
                            if not self.water_change_active.get(sid, False):
                                ...
                                if current is not None:
                                    print(f"[SCHEDULER] Triggering scheduled water change for Tank {sid}")
    ...
    def open_water_change_popup(self, default_sensor="A"):
        ...
        def _stop():
            ...
            # IMMEDIATE SAFETY STOP: Cut relay first
            try:
                import RPi.GPIO as GPIO
                GPIO.output(self.drain_gpio, GPIO.LOW)
                print(f"[SAFETY] Drain relay cut immediately for Tank {sid}")
            except Exception as e:
                print(f"[ERROR] Safety cut failed: {e}")

            ok = self.show_confirm("Stop draining?", "The drain has been paused. Stop it permanently?", yes_text="Stop", no_text="Resume")

            if ok:
                self.stop_water_change(sid, reason="manual")
                _close()
            else:
                # RESUME: Turn relay back ON
                try:
                    import RPi.GPIO as GPIO
                    GPIO.output(self.drain_gpio, GPIO.HIGH)
                    print(f"[SAFETY] Drain resumed for Tank {sid}")
                except Exception as e:
    ...
    def water_change_tick(self, sensor_id: str, current_mmwg: float):
        ...
        try:
            ...
            if float(current_mmwg) <= float(target):
                ...
                self.safe_gui_update(lambda sid=sid: self.show_info("Water change", f"Tank {sid}: Drain Complete. Auto top-up if enabled will now continue."))
        except Exception:
            pass
