# Stork Aquatics Monitor Max V1.3.8
# Wi-Fi TCP - [Enabled]
# 4Ch Relay - [Enabled]
# RO System 12h auto power off override - [Enabled]
# Frames Adjustments & Hiding (See Lines 245-254 for grid moving & 74-82 "True"= Shown "False"= Hidden
...
class SensorGUI:
    def __init__(self, root):
        self.root = root
        # GUI Setup
        self.root.title("Stork Aquatics Monitor Max V1.3.8")
        ...
        # Main Grid Layout Adjustments Here
        self.aquarium_frame_1 = self.create_sensor_frame("Aquarium A", 0, 0)
        self.aquarium_frame_2 = self.create_sensor_frame("Aquarium B", 1, 0)
        self.ro_tank_frame = self.create_ro_tank_frame("RO Tank", 1, 2)
        self.ph_level_frame = self.create_ph_level_frame("pH Sensor", 0, 2)
        self.pump_frame_a = self.create_pump_frame("RO Pump A", 0, 1)
        self.pump_frame_b = self.create_pump_frame("RO Pump B", 1, 1)
        self.tds_level_frame = self.create_tds_level_frame("TDS Sensor", 2, 0)
        self.image_frame_b = self.create_image_frame_b("", 2, 1)
        self.image_frame_c = self.create_image_frame_c("www.stork.solutions", 2, 2)
        ...
        try:
            if auto_mode:
                ...
                if elapsed >= self.KEEPALIVE_WINDOW_SECS and not self.anti_idle_active.get(pump_name, False):
                    # Only cycle if pump is actually ON; otherwise there's nothing to "power blip"
    ...
    def create_tds_level_frame(self, title, row, column, colspan=1):
        ...
        settings_button = tk.Button(frame, text="Settings", command=self.open_tds_settings_popup)
        settings_button.pack(pady=5)
    ...
    def open_tds_settings_popup(self):
        ...
        def save_tds_settings():
            try:
                ...
                # FORCE a layout pass by resetting the cache, then re-layout now
                self.safe_gui_update(self.layout_tds_tile)

                # (optional) pull fresh values right away
                try:
    ...
    def apply_reading_colors(self):
        try:
            ...
            self.tds_level_frame["connection_status"].config(fg="green")
            ...
            try:
                ...
                if sensor_id == "A":
                    ...
                    def _apply():
                        ...
                        # If you want to visually emphasize the selected mode, you could bold it:
                        # reset fonts
                        base = ("Arial", 14, "bold")
    ...
    def layout_tds_tile(self):
        ...
        if self._tds_last_visibility == vis_state:
            # No change -> do not thrash the layout
            return
        ...
        # First, forget everything so we can re-pack in a clean, fixed order
